...
STATS_FILE = "checker_stats.json"  # برای آمار عبور موفق کاربران
...
def handle_start(message):
    ...
    if len(args) > 1:
        ...
        if not non_members:
            save_passed_user(message.from_user.id)
...
def check_again(call):
    ...
    if not non_members:
        save_passed_user(call.from_user.id)
...
def save_passed_user(user_id):
    ...
    if user_id not in stats[today]:
        stats[today].append(user_id)
